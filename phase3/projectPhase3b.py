...
#rmse lists for 5 old algos + 4 cf variants
rmse_random = []
rmse_user = []
rmse_movie = []
rmse_dem = []
...
for rep in range(10):
    ...
    #step 1: split ratings into training and test
    trainingSet, testSet = partitionRatings(rawRatings, 20)

    #step 2: build rating data structures from training set only
    numUsers = len(userList)
    numMovies = len(movieList)
    trainingRLu, trainingRLm = createRatingsDataStructure(numUsers, numMovies, trainingSet)

    #precompute neighbor lists for each user for this repetition
    neighbors10 = {}
    neighbors100 = {}
    neighbors500 = {}
    neighborsAll = {}
    for u in range(1, numUsers + 1):
        all_neighbors = kNearestNeighbors(u, trainingRLu, numUsers - 1)
        neighborsAll[u] = all_neighbors
        neighbors10[u] = all_neighbors[:10]
        neighbors100[u] = all_neighbors[:100]
        neighbors500[u] = all_neighbors[:500]

    #lists to store predicted and actual ratings for this repetition
    actual = []
    pred_random = []
    pred_user = []
    pred_movie = []
    ...
    #step 3: loop through test set and get predictions
    for (u, m, r) in testSet:
        ...
        #4 collaborative filtering variants
        friends10 = neighbors10[u]
        friends100 = neighbors100[u]
        friends500 = neighbors500[u]
        friendsAll = neighborsAll[u]

        pred_cf10.append(CFRatingPrediction(u, m, trainingRLu, friends10))
        pred_cf100.append(CFRatingPrediction(u, m, trainingRLu, friends100))
        pred_cf500.append(CFRatingPrediction(u, m, trainingRLu, friends500))
        pred_cf_all.append(CFRatingPrediction(u, m, trainingRLu, friendsAll))

    #step 4: compute rmse for this repetition and store
    ...
    rmse_cf10.append(rmse(actual, pred_cf10))
    rmse_cf100.append(rmse(actual, pred_cf100))
    rmse_cf500.append(rmse(actual, pred_cf500))
    rmse_cf_all.append(rmse(actual, pred_cf_all))

#step 5: make boxplot of all 9 algorithms
data = [rmse_random, rmse_user, rmse_movie, rmse_dem, rmse_genre,
        rmse_cf10, rmse_cf100, rmse_cf500, rmse_cf_all]

plt.boxplot(data, patch_artist=True)
plt.xticks([1,2,3,4,5,6,7,8,9],
           ['random','user','movie','dem','genre',
            'cf-10','cf-100','cf-500','cf-all'])
plt.ylabel('RMSE')
